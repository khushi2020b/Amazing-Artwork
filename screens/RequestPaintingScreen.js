...
export default class ReadStoryScreen extends React.Component(){
  ...
  retreivePaintings(){
      try{
          var allPaintings= [];
          var paintings = db.collection("available_paitings")
          .get().then((querySnapshot) =>{
              querySnapshot.forEach((doc) =>
              {
                  allPaintings.push(doc.data())
                  console.log('These are the Paintings you can purchase - ', allPaintings)
              })
  ...
  getPaintingRequest = () => {
    var paintingRequest = db
      .collection("requested_paintings")
            ...
            .then((snapshot) => {
              snapshot.forEach((doc) => {
                ...
                db.collection("all_notifications").add({
                  ...
                  message:
                    username + " received the painting " + paintingName,
                ...
                renderItem = {({item}) =>(
                  <View>
                    ...
                    <View style = {styles.paintingDetails}>
                      ...
                      <Text style = {style.cost}>Cost of Painting: {this.state.cost}</Text>
                      <TouchableOpacity 
                      style = {styles.button}
                      onPress = {
                        this.navigation.navigate("PaintingDetailsScreen")
                      }>
